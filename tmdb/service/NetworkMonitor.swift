...
/// A protocol that defines the capabilities of a network connectivity monitor.
/// This abstraction allows for easily swapping between a real network monitor and mock implementations for testing.
protocol NetworkMonitorProtocol {
    /// Indicates whether there is an active network connection.
    var isConnected: Bool { get }
    /// Provides the detailed status of the network path.
...
class NetworkMonitor: NetworkMonitorProtocol {
    ...
    private(set) var isConnected: Bool = true // Assume connected until verified
    ...
    private(set) var connectionStatus: NWPath.Status = .satisfied // Assume satisfied until verified
